...
import type { MappExpoPluginProps, NormalizedMappExpoPluginProps } from './types';

const prefix = '[react-native-mapp-plugin]';

function fail(property: string, expectation: string): never {
  throw new Error(`${prefix} ${property} ${expectation}. Configure it in the Mapp plugin options.`);
}

function nonEmptyString(value: unknown, property: string): asserts value is string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    fail(property, 'must be a non-empty string');
  }
}

function optionalBoolean(value: unknown, property: string): void {
  if (value !== undefined && typeof value !== 'boolean') {
    fail(property, 'must be a boolean');
  }
}

export function validateAndNormalizeProps(
  config: ExpoConfig,
  props: MappExpoPluginProps
): NormalizedMappExpoPluginProps {
  ...
  if (pushHandling === 'mapp' && !config.android.googleServicesFile) {
    fail(
      ...
      'must point to the Firebase Android application google-services.json when android.pushHandling is "mapp"'
    );
  }
  ...
  if (ios.customFields !== undefined && (
    !Array.isArray(ios.customFields) ||
  ...
  if (ios.mediaTimeout !== undefined && (
    typeof ios.mediaTimeout !== 'number' || !Number.isFinite(ios.mediaTimeout) || ios.mediaTimeout <= 0
  )) {
